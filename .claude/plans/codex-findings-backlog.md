...
- 2026-07-04 | MEDIUM | .claude/plans/comax-secrets-m7-infra-config-templating.plan.md | (plan-codex R1 F3) `secret render`의 활성 config in-place 안전 교체 — PoC는 staging-only로 한정. backup/rollback/fsync/OS별 atomic-replace + 실패주입 테스트로 라이브 redis.conf/nginx.conf를 무중단 교체하는 경로는 M7 결정이 In일 때 v1-full에서 구현.
- 2026-07-05 | HIGH | .claude/plans/comax-secrets-m7-infra-config-templating.plan.md | (implement-codex R1 impl-F2) 서버측 env-name 검증(`internal/server/validate.go`)에서 `self`를 예약어로 hard reject — PoC는 서버 변경 0 유지를 위해 render-side 결정론 alias + 가드 에러로만 방어. M7 결정이 In이면 서버 create/update에서 `self` env 생성 거부를 추가해 alias 충돌 원천 차단.
