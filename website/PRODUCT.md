...
- **대담함**: Bun류 인디 캐릭터. 뚜렷한 POV로 "이건 가볍고, self-host고, 네 문제를 푼다"를 자신 있게 말한다. 단, 대담함은 유채색이 아니라 **구조·실증(인터랙티브 데모)·타이포 리듬**으로 낸다. 팔레트는 대시보드의 monochrome graphite + 단일 blue 액센트를 계승해 "한 제품"감을 지킨다("색은 의미에만").
...
2. **대담하되 근거 있게** — Bun류 인디 캐릭터(구조·타이포·실증)를 쓰되, 모든 대담함은 구체적 주장으로 갚는다. buzzword·hero-metric·과장은 금지. 진짜 숫자와 진짜 명령으로 말한다.
3. **크래프트가 신뢰다** — 시크릿 도구의 신뢰는 광택이 아니라 손으로 맞춘 디테일에서 온다. 타이포 리듬·코드블록·상태·간격의 완성도가 곧 브랜드다.
4. **한 제품** — 랜딩은 대시보드의 monochrome + 단일 blue 액센트 시스템을 그대로 계승한다. 뉴트럴·시맨틱 토큰 전부 parity(예외 없음, token-parity 계약 그대로). 차별화는 색이 아니라 랜딩만의 구조 — 수렴 그래픽·인터랙티브 데모·before/after 내러티브 — 로 낸다.
